...
<<<<<<< HEAD
      if (typeof window !== "undefined") {
        ...
        try {
          window.localStorage.setItem("rm:complaints:update", updateStamp);
        } catch {
          // Ignore storage failures and continue with the in-tab signal.
        }

        window.dispatchEvent(new Event("rm:complaints:update"));

        if ("BroadcastChannel" in window) {
          const channel = new BroadcastChannel("rm:complaints");
          channel.postMessage({ type: "updated", timestamp: updateStamp });
          channel.close();
        }
      }
=======
