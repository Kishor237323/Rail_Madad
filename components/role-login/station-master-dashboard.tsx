...
export function StationMasterDashboard({ username }: { username: string }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/station-master/assigned?username=${encodeURIComponent(username)}`);
        const data = (await response.json()) as Partial<StationResponse>;
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">🏢 Station Master Dashboard</h1>
        <p className="mt-1 text-slate-600">Emergency handling and station preparation view.</p>
        {profile ? (
          <div className="mt-3 rounded-xl border border-slate-200 bg-white p-3 text-sm text-slate-700">
            <p>👤 {profile.name}</p>
            <p>🏢 Role: {String(profile.role || "station_master").replace("_", " ")}</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="rounded-2xl border-slate-200">
          <CardContent className="p-4">
            ...
            <p className="mt-1 text-2xl font-bold text-red-600">{loading ? "..." : emergencyAlerts}</p>
          </CardContent>
        </Card>
        <Card className="rounded-2xl border-slate-200">
          <CardContent className="p-4">
      ...
      {loading ? (
        <Card className="rounded-2xl border-slate-200">
          <CardContent className="p-6 text-sm text-slate-500">Loading emergency complaints...</CardContent>
        </Card>
      ) : complaints.length === 0 ? (
        <Card className="rounded-2xl border-dashed border-slate-300">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-lg font-semibold text-slate-700">No emergency complaints yet</p>
            <p className="text-sm text-slate-500">Emergency Alerts: 0 · Pending: 0 · In Progress: 0 · Resolved: 0</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-3">
          {complaints.map((complaint) => {
            const isMedical = complaint.category.toLowerCase() === "medical";
            const categoryIcon = isMedical ? HeartPulse : Flame;
              ...
              <Card key={complaint.complaintId} className="rounded-2xl border-red-200 bg-red-50/30">
                ...
                <CardContent className="space-y-2 text-sm text-slate-700">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge className="inline-flex items-center gap-1 bg-red-100 text-red-700 hover:bg-red-100">
