...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    const handleStorageUpdate = (event: StorageEvent) => {
      if (event.key === "rm:complaints:update") {
        refresh();
      }
    };

    const handleBroadcastMessage = (event: MessageEvent) => {
      if (event.data && typeof event.data === "object" && (event.data as { type?: string }).type === "updated") {
        refresh();
      }
    };

    const complaintChannel = "BroadcastChannel" in window ? new BroadcastChannel("rm:complaints") : null;
    const handleWindowUpdate = () => refresh();

    const intervalId = window.setInterval(refresh, 30_000);
    window.addEventListener("focus", handleWindowUpdate);
    window.addEventListener("visibilitychange", handleWindowUpdate);
    window.addEventListener("storage", handleStorageUpdate);
    window.addEventListener("rm:complaints:update", handleWindowUpdate as EventListener);
    complaintChannel?.addEventListener("message", handleBroadcastMessage);

    return () => {
      window.clearInterval(intervalId);
      window.removeEventListener("focus", handleWindowUpdate);
      window.removeEventListener("visibilitychange", handleWindowUpdate);
      window.removeEventListener("storage", handleStorageUpdate);
      window.removeEventListener("rm:complaints:update", handleWindowUpdate as EventListener);
  ...
  const displayedTrendData = useMemo(() => {
    if (trendRange === "7") return trendData.slice(-7);

    if (trendRange === "30") return trendData.slice(-30);

    const last90Days = trendData.slice(-90);
    const weeklyBuckets: Array<{ date: string; complaints: number; resolved: number }> = [];

    for (let i = 0; i < last90Days.length; i += 7) {
      const chunk = last90Days.slice(i, i + 7);
      const firstPoint = chunk[0];

      weeklyBuckets.push({
        date: firstPoint?.date || `Week ${weeklyBuckets.length + 1}`,
        complaints: chunk.reduce((sum, item) => sum + item.complaints, 0),
        resolved: chunk.reduce((sum, item) => sum + item.resolved, 0),
      });
    }
  ...
  const trendChartData = displayedTrendData.length > 0 ? displayedTrendData : fallbackTrendData;
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                type="button"
                size="sm"
                variant={trendRange === "90" ? "default" : "outline"}
          ...
          <div style={{ height: 350 }}>
            {!isClient || loading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart key={trendRange} data={trendChartData}>
                  <defs>
                    <linearGradient id="colorComplaints" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#0B3C5D" stopOpacity={0.35} />
                  ...
                  <YAxis tick={{ fontSize: 11 }} tickLine={false} axisLine={false} domain={[0, "dataMax + 1"]} allowDecimals={false} />
                  <Tooltip 
                    contentStyle={{ 
